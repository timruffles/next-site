...
- `routeChangeError(err, url)` - Fires when there's an error when changing routes (it's advisable to define this if you
  define any of the other event listeners; exceptions thrown in them will be swallowed and prevent route changes)
